    ...
    def build(operations):
        ...
        for operation in range(0,len(operations)):
            if operations[operation]["Name"] == "EmptyFolderContents":
                ...
                if "Schema" in operations[operation]: #select specific properties of an item. mailbox store only. cannot use for attachments.
                ...
                if "Range" in operations[operation]: #select bytes to fetch. only for documents and attachments.
                ...
                if "UserName" in operations[operation]: #select username and password to use for fetch. i imagine this is  only for documents.
                    if not "Password" in operations[operation]:
